...
for file in files:
    if '.fzm' in file:
        fzmFile = os.path.join(filepath,file)
    if 'Specs.csv' in file:
        SpecFile = os.path.join(filepath,file)
    if 'InputOutputs.py' in file:
        DoutputsFile = os.path.join(filepath,file)
    if 'DefaultValues.py' in file:
        DefaultValFile = os.path.join(filepath,file)
    if 'ScopeCHN.csv' in file:
        ScopeFile = os.path.join(filepath,file)
...
def Explore():
    ...
    for line in open(os.path.join(scriptFilePath,'ExploreSettings.py'),'r').readlines():
        print(line)
        exec(line.replace('\n',''),globals())
    # print(SpecFile)
...
def CreateModelInformationSection():
    ...
    os.system('move BehavioralModel.pdf "Behavioral Model\\BehavioralModel.pdf"')
    os.system('move "Behavioral Actions.tex" "Behavioral Model\\cBehavioral Actions.tex"')
    os.system('robocopy "%sBehavioral Model" "%sreport\\data\\Behavioral Model"' % (scriptFilePath,filepath))
    file.write('robocopy "%sBehavioral Model" "%sreport\\data\\Behavioral Model"' % (scriptFilePath,filepath) + '\n')
    ...
    os.system('move ExploredModel.pdf "Explored Model\\aExploredModel.pdf"')
    os.system('move "State Output Tablestatetable.csv" "Explored Model\\Explored Model State Output Table Reportstatetable.csv"')
    os.system('move "State Transition Tablestatetable.csv" "Explored Model\\Explored Model State Transition Table Reportstatetable.csv"')
    os.system('robocopy "%sExplored Model" "%sreport\\data\\Explored Model"' % (scriptFilePath,filepath))
    if useScope:
     os.system('robocopy "%sMeasurements" "%sreport\\data\\Measurements"' % (scriptFilePath,filepath))
    os.system('robocopy "%sTesting Strategy" "%sreport\\data\\Generating Tests"' % (scriptFilePath,filepath))
    file.close()
